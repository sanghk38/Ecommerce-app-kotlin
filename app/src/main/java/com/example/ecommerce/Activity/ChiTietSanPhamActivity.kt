...
class ChiTietSanPhamActivity : AppCompatActivity() {
    ...
    private fun EventButton() {
        bntDatmua.setOnClickListener(View.OnClickListener {
            ...
            for (i in HomeActivity.manggiohang.indices){
                ...
                if (exite == false){
                    var soluong = spinner.selectedItem.toString().toInt()
                    var Giamoi = (soluong * Giachitiet).toLong()
                    HomeActivity.manggiohang.add(
                        GioHang(id, Tenchitiet,Giamoi,Hinhanhchitiet,soluong,kt.toInt())
                    )
                }
            }else{
                var soluong = spinner.selectedItem.toString().toInt()
